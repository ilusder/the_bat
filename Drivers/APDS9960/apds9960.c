 ...
 * @return True if successful write operation. False otherwise.
 */
uint8_t APDS9960_wireWriteDataByte(I2C_HandleTypeDef, uint8_t reg, uint8_t * val)
{
...
uint8_t APDS9960_wireReadDataByte(I2C_HandleTypeDef *hi2c, uint8_t reg, uint8_t * val)
{
    ...
    if (!APDS9960_wireWriteByte(hi2c, *val)) {
        return HAL_ERROR;
    }
                                        ...
                                        unsigned int len)
{
    ...
    /* Indicate which register we want to read from */
    if (!APDS9960_wireWriteByte(hi2c)) {
        return -1;
    }
